...
# list for parallel shedulling : 
collected_product_ids = []
collected_product_features = []
collected_product_quantity = []
collected_product_future_features = []


# Process data for arima models :
...
def process_data_ARIMA(Product_features_json, Product_quantity_json, Product_future_features_json, Product_Id_produit_json) :
    ...
    # print(json_output)
    return x_future, final_df, target, nb_jours, exogenous , pd.DataFrame(Product_features_json), Product_Id_produit_json




@app.route('/api/modelbooper/prixpermanent/collect_product_id', methods=['POST'])
def receive_data():
    ...
    if Product_features_json and Product_quantity_json and Product_future_features_json and Product_Id_produit_json :
        collected_product_ids.append(Product_Id_produit_json)
        collected_product_features.append(Product_features_json)
        collected_product_future_features.append(Product_future_features_json)
        collected_product_quantity.append(Product_Id_produit_json)

        return jsonify({"message": "Product ID collected"}), 200
    
    else:
        return jsonify({"error": "No product ID provided"}), 400


@app.route('/api/modelbooper/prixpermanent/process_collected_ids', methods=['GET'])
def process_collected_ids():
    if collected_product_ids:
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = []
            for i in range(len(collected_product_ids)):
                product_id = collected_product_ids[i]
                product_features = collected_product_features[i]
                product_future_features = collected_product_future_features[i]
                product_quantity = collected_product_quantity[i]

                x_future, final_df, target, nb_jours, exogenous = process_data_ARIMA(product_features, product_quantity, product_future_features, product_id)
                future = executor.submit(process_product_ARIMA, x_future, final_df, target, nb_jours, exogenous, product_id)
                futures.append(future)

            results = [future.result() for future in futures]

        # Clear the collected data after processing
        collected_product_ids.clear()
        collected_product_features.clear()
        collected_product_future_features.clear()
        collected_product_quantity.clear()
