...
#Ancienne elasticté pas la vrai mais parts avec shap et importance 
...
# #Nouvelle fonction la vraie elasticité pas shap ou importance 
# # ── 1. ÉLASTICITÉS sur le passé (perturbation sur final_df) ───────────────
def compute_elasticites(final_df, target, exogenous, model):
    ...
    for feat in exogenous:
        ...
        # ── BINAIRES : 0/1 → semi-élasticité (ΔQ/Q) / ΔX, ΔX=1 ──────────
        if feat in BINAIRES or feat.startswith('PARAM_JS_'):
            ...
            # Semi-élasticité : pas de division par X car X=0→1
            e = float((Q_1.sum() - Q_0.sum()) / Q_sum_0)
            e = max(min(e, 10.0), -1.0)

        # ── CONTINUS : e = (ΔQ/Q) / (ΔX/X) ──────────────────────────────
        else:
            ...
            # Perturbation +1% des valeurs réelles (PROMO=0.30 → 0.303)
            # Les lignes à 0 restent à 0 → ne contribuent pas au numérateur ✓
            df_p        = df.copy()
            df_p[feat]  = df_p[feat] * 1.01

            # Clip PROMO à 1.0 max
            if 'PARAM_PROMO' in feat:
                df_p[feat] = df_p[feat].clip(upper=1.0)

            Q_p  = model.predict(df_p[exogenous])
            dQ   = (Q_p.sum() - Q_base.sum()) / Q_sum   # ΔQ/Q
            e    = float(dQ) / 0.01                      # / (ΔX/X) = 0.01
...
#     df = final_df.copy()
#     Q_mean = float(df[target].mean()) or 1.0

#     explainer = shap.TreeExplainer(model)
#     shap_vals = explainer.shap_values(df[exogenous])

#     result = {}
#     for i, feat in enumerate(exogenous):
#         if feat in SKIP:
#             continue

#         signed_mean = float(shap_vals[:, i].mean())
#         elasticity = round(signed_mean / Q_mean, 4)

#         # ── Protections métier ──────────────────────────────────────
#         if feat == 'PARAM_PRIX' and elasticity > 0:
#             elasticity = 0.0
#         if 'PARAM_PROMO' in feat and elasticity < 0:
#             elasticity = 0.0
#         if 'PARAM_CONC' in feat and elasticity < 0:
#             elasticity = 0.0
#         # ────────────────────────────────────────────────────────────

#         result[feat] = elasticity
...
def GET_process_product_Version(x_future, final_df, target, nb_jours, exogenous, id_produit, id_so, date_import, model_name, feature_Model, parm_model):
    ...
    #Fourcette +/- 20% et dernier prix au milieu
    vec_prix_test = [last_price * f for f in [0.80, 0.90, 1.00, 1.10, 1.20]]
    ...
    coefficients = compute_elasticites(final_df, target, exogenous, model)
