...
def version_SET(Product_features_json, Product_quantity_json, Product_future_features_json, ID_SO_request, Product_Id_produit_json, date_import) : 

    x_future, final_df, target, nb_jours, exogenous = AllModels.process_data(Product_features_json, Product_quantity_json, Product_future_features_json, Product_Id_produit_json)
    print(f"L{Product_Id_produit_json}")
    results = AllModels.SET_process_product_Version(x_future, final_df, target, nb_jours, exogenous, Product_Id_produit_json, ID_SO_request, date_import)
...
def receive_data2():
    ...
    if len(Product_features_json) != 0 and len(Product_quantity_json) != 0 : 
        ...
        if features_model != "-1" and parm_model != "-1" and date_import != "-1 " and model_name != "-1": 
            print(f"Je charge un ancien modèle")
            ...
            result =  version_SET(Product_features_json, Product_quantity_json, Product_future_features_json, So_Id_json, Product_Id_produit_json, date_import)
...
def receive_data():
    ...
    if len(Product_features_json) != 0 and len(Product_quantity_json) != 0 : 
        ...
        if len(Product_features_json) != 0 and len(Product_quantity_json) != 0 : 
            ...
            result = version_SET(Product_features_json, Product_quantity_json, Product_future_features_json, ID_SO_request, Product_Id_produit_json, date_import)
